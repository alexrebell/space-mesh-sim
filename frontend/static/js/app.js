...
function createOrbit(options) {
  ...
  const phaseStepRad =
    phaseStepDeg > 0 ? phaseStepDeg * DEG2RAD : null;
...
// Возвращаем entity орбиты (полилинию), чтобы потом уметь удалять
// Здесь орбита рисуется в фиксированной плоскости — это нормальная визуализация
...
// ВАЖНО: здесь уже учитывается вращение Земли (OMEGA_E),
// чтобы ground track был с межвитковым сдвигом, а наземные станции видели разные витки.
function createSatelliteOnOrbit(orbit, color, satIndex, totalSatellites) {
  ...
  const satEntity = viewer.entities.add({
    ...
    // без label — карта остаётся чистой
...
function addOneSatelliteToOrbit(orbitId) {
  ...
  const newSatIndex = totalNow; // следующий индекс
...
function renderOrbitList() {
  ...
  orbitStore.forEach((group) => {
    ...
    // цветная полоска слева в цвет орбиты
    if (group.cssColor) {
    ...
    const totalSats =
      group.satellites.length > 0
        ...
        : group.orbit.numSatellites;

    let deltaThetaRad;
    if (
      group.orbit.evenSpacing ||
      !group.orbit.phaseStepRad ||
      group.orbit.phaseStepRad <= 0
    ) {
...
viewer.selectedEntityChanged.addEventListener(function (entity) {
  ...
  // Нас интересуют только спутники
  const props = entity.properties;
  if (!props) return;

  let isSat = false;
  try {
    // properties.isSatellite может быть либо простым значением, либо Property
...
clock.onTick.addEventListener(function (clock) {
  ...
  if (seconds - lastLinkInfoUpdateSeconds < 0.5) {
    return;
  }
...
<!--        Значения обновляются в реальном времени и могут использоваться для расчёта FSPL / RSSI / SNR.-->
<!--      </div>-->
    `;
  } else {
    linkInfoBody.innerHTML = `
...
if (orbitForm) {
  orbitForm.addEventListener("submit", function (e) {
    ...
    if (
      ...
      console.error(
        "Не найдены элементы формы орбиты. Проверь id полей в index.html"
      );
      return;
    }

    const name = nameInput.value || "LEO-custom";
    const altitudeKm = parseFloat(altitudeInput.value) || 500;
    const inclinationDeg = parseFloat(inclinationInput.value) || 53;
    const numSatellites = parseInt(numSatsInput.value) || 1;

    const evenSpacing = evenSpacingInput.checked;
    const phaseStepRaw = parseFloat(phaseStepInput.value);
    const phaseStepDeg = isNaN(phaseStepRaw) ? 0 : phaseStepRaw;
...
if (bulkForm) {
  bulkForm.addEventListener("submit", function (e) {
    ...
    const inclStepInput = document.getElementById("bulk-incl-step");
    ...
    const altitudeKm = parseFloat(altInput.value) || 550;
    const numSatellites = parseInt(numSatsInput.value) || 8;
    const evenSpacing = evenSpacingInput.checked;
    const phaseStepRaw = parseFloat(phaseStepInput.value);
    const phaseStepDeg = isNaN(phaseStepRaw) ? 0 : phaseStepRaw;
    let inclStep = parseFloat(inclStepInput.value) || 10;
    const skipPolar = skipPolarInput.checked;

    if (inclStep <= 0) inclStep = 10;
    if (inclStep > 180) inclStep = 180;

    // Генерация орбит от 0 до 180 включительно с шагом inclStep
    for (let incl = 0; incl <= 180 + 1e-6; incl += inclStep) {
      const inclRounded = Math.round(incl * 1000) / 1000; // на всякий случай
